...
class LocalVideoTranscriber:
    def __init__(self, video_folder="stream_video", output_csv="stream_video_transcripts.csv"):
        self.video_folder = video_folder
        self.output_csv = output_csv
        self.video_info_list = []
        self.today_str = datetime.datetime.now().strftime("%Y%m%d")
        # 載入 Nemo ASR 模型並轉為半精度以降低 VRAM 使用量
        self.asr_model = nemo_asr.models.ASRModel.from_pretrained("nvidia/canary-1b-flash")
        self.asr_model = self.asr_model.half()
        # 將模型包裝在 DataParallel 中
        self.asr_model = torch.nn.DataParallel(self.asr_model)

    def transcribe_videos(self):
        """
        逐一處理 stream_video 資料夾中的影片，
        將影片音訊轉為單聲道後使用 Nemo ASR 模型進行語音轉文字，
        ...
        for filename in os.listdir(self.video_folder):
            ...
            if os.path.isfile(file_path) and filename.lower().endswith(('.mp4', '.mkv', '.mov', '.avi')):
                ...
                # 透過 module 屬性呼叫原始模型的 transcribe 方法
                transcriptions = self.asr_model.module.transcribe([mono_audio_file], batch_size=1, num_workers=1)
